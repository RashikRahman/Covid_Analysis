...
def plot_exponential_fit_data(d_df, title, delta):
    ...
    g = sns.scatterplot(x=d_df['x'][:-delta], y=d_df['y'][:-delta], label='Confirmed cases (used for model creation)',
                        color='red')
    g = sns.scatterplot(x=d_df['x'][-delta:], y=d_df['y'][-delta:],
                        label='Confirmed cases (not used for model, validation)', color='blue')
...
d_df = CVD_BD.copy()
plot_exponential_fit_data(d_df, 'Bangladesh', 3)
